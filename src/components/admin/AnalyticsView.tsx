/**
 * AnalyticsView — Dashboard principal del admin de Martes Hub.
 *
 * Payload custom admin view registrada en payload.config.ts en `/admin/analytics`.
 * Porta el shell visual de storelink adaptado al modelo de datos de Martes Hub:
 *   - orders/products/customers  →  payments/leads/clients
 *
 * Auth: getWorkspaceContext() resuelve usuario + tenant activo con access control.
 * Datos: todas las queries usan overrideAccess: false + user (aislamiento por tenant).
 */
...
/** Last `n` days oldest→newest, each with a Spanish day label and an amount bucket. */
...
export async function AnalyticsView() {
  const { payload, user, tenant, tenantId, isAdmin } = await getWorkspaceContext()
    ...
    <div className="min-h-screen font-sans antialiased text-zinc-100 bg-black selection:bg-white selection:text-black">
        ...
        <div className="mx-auto flex min-h-14 max-w-[1600px] flex-wrap items-center justify-between gap-4 px-4 py-2.5 sm:px-6 xl:px-8">
          ...
          <nav className="order-3 flex w-full overflow-x-auto border border-zinc-800 bg-zinc-950 p-0.5 lg:order-none lg:mx-auto lg:w-auto">
            <Link
              ...
              className="shrink-0 px-3.5 py-1 text-xs font-bold bg-white text-black uppercase tracking-wider"
            >
              Dashboard
            </Link>
            <Link
              href="/admin/collections/leads"
              className="shrink-0 px-3.5 py-1 text-xs font-medium text-zinc-400 hover:text-white hover:bg-zinc-900 uppercase tracking-wider transition"
            >
              Leads
            </Link>
            <Link
      ...
      <main className="mx-auto max-w-[1600px] space-y-5 px-4 py-5 sm:px-6 xl:px-8">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.4fr_.8fr]">
          ...
          {/* Bar chart: leads últimos 7 días */}
                ...
                [
                  {
                    ...
                    href: '/admin/collections/leads?where[status][equals]=nuevo',
                  },
                  {
                    label: 'Contactado',
                    count: leadsContactado.totalDocs,
                    href: '/admin/collections/leads?where[status][equals]=contactado',
                  },
                  {
                    label: 'Calificado',
                    count: leadsCalificado.totalDocs,
                    href: '/admin/collections/leads?where[status][equals]=calificado',
                  },
                  {
                    label: 'Convertido',
