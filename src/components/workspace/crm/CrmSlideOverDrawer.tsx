...
const inputCls =
  'w-full border border-border bg-background px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-zinc-500 transition-colors font-sans'
const labelCls = 'flex flex-col gap-1.5 text-[11px] font-mono uppercase tracking-wider text-muted-foreground'
...
const STATUS_OPTIONS: { value: LeadStatus; label: string; badgeCls: string }[] = [
  ...
  { value: 'descartado', label: 'Descartado', badgeCls: 'border-border bg-muted text-muted-foreground' },
]
...
export function CrmSlideOverDrawer({
  ...
  const btnCls =
    variant === 'primary'
      ...
      : variant === 'ghost'
        ? 'inline-flex items-center gap-1 text-[10px] font-mono text-muted-foreground hover:text-foreground px-1.5 py-0.5 border border-border hover:border-zinc-600 bg-muted transition'
        : variant === 'inline'
          ? 'inline-flex items-center gap-1 rounded text-[11px] font-mono font-medium text-zinc-300 hover:text-foreground transition'
          : 'px-3.5 py-2 bg-muted hover:bg-zinc-800 border border-zinc-700 text-zinc-200 font-bold flex items-center gap-2 uppercase transition text-xs font-mono'
        ...
        >
          <SheetHeader className="border-b border-border px-4 py-3">
            ...
            <SheetDescription className="sr-only">
              Formulario de alta de {entityTitle}
            </SheetDescription>
          </SheetHeader>
          <div className="flex flex-1 flex-col overflow-y-auto p-4">
            <div className="flex flex-col gap-5 pb-6">
              {/* Subheader descriptivo con badge de radar */}
              <div className="border border-zinc-850 bg-muted/40 p-3">
              ...
              >
                          ...
                          <button
                            ...
                            className={`px-2.5 py-2 text-center text-xs font-mono font-medium border transition-all ${
                              isSelected
                                ...
                                : 'border-border bg-muted/50 text-muted-foreground hover:border-zinc-700 hover:text-zinc-200'
                            }`}
                          >
                            {opt.label}
                          </button>
                ...
                <div className="border border-zinc-850 bg-background p-3.5 flex flex-col gap-3">
                ...
                <div className="border border-zinc-850 bg-background p-3.5 flex flex-col gap-2">
                  <label className={labelCls}>
                ...
                {/* Footer de Acciones */}
                <div className="flex items-center justify-between gap-3 border-t border-border/80 pt-4 mt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={handleClose}
                      className="px-4 py-2 bg-muted hover:bg-zinc-800 border border-zinc-700 text-foreground text-xs font-bold uppercase tracking-wider font-mono transition"
                    >
                      Cancelar
                    </button>
                    <button
