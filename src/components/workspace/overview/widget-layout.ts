...
export function readStoredWidgetLayout<K extends string>(
  storageKey: string,
  validKeys: readonly K[],
): WidgetConfig<K>[] | null {
  try {
...
/**
 * Hook de layout del Bento persistido en localStorage.
 *
 * Crítico para la hidratación: el estado inicial SIEMPRE es el default, de
 * modo que el render del servidor y el primer render del cliente producen el
 * mismo árbol. El layout guardado se restaura en un efecto posterior a la
 * hidratación; si el estado inicial dependiera de localStorage (solo
 * disponible en cliente), React recibiría árboles distintos y fallaría.
 */
export function useWidgetLayout<K extends string>(
  ...
  const [widgets, setWidgets] = useState<WidgetConfig<K>[]>(defaults)

  useEffect(() => {
    const stored = readStoredWidgetLayout(storageKey, validKeys)
    if (stored) setWidgets(stored)
